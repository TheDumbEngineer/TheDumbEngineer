...
read -p "Enter the COM port for your Raspberry Pi Pico (e.g., COM3 for Windows, /dev/ttyACM0 for Linux): " COM_PORT

# Check if the COM port is provided
if [ -z "$COM_PORT" ]; then
    echo "COM port cannot be empty. Aborting."
    exit 1
fi
...
rp2.py -p "$COM_PORT" -b 115200 -f "$FIRMWARE_PATH"
